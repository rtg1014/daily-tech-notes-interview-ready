...
## 연쇄적으로 이해가 안되는 부분들 추가학습

> ## 세번째 의문

### 의문점
```
데이터처리를 빠르게할것도 아니고 무결성을 위해서 트랜젝션을 사용한다? REPEATABLE-READ 랑 SERIALIZABLE 에 해당하는 예시는
계좌잔액수정이나 결제, 금융에 사용되니까 이해가 되는데,,
그런데 READ-UNCOMMITTED, READ-COMMITTED 의 사용예시가  로그데이터, 통계수집, 사용자 도회, 상품리스트 조회?
이런 내용들은 단순한 셀렉트문으로 해도 아무런 문제가 없는것들이 아니였을까?
그럼 굳이 트랜젝션 단위로 할 필요가 있어? 그럼 결국 저 2가지 고립단계는 결국 무쓸모인거 아닌가?
```
...
위 의문점에 대해 **READ-UNCOMMITTED**와 **READ-COMMITTED**가 왜 존재하는지, 그리고 정말 "무쓸모"인지에 대해 구체적인 설명은 다음과 같다.

---

### **1. READ-UNCOMMITTED와 READ-COMMITTED는 왜 필요한가?**

#### **READ-UNCOMMITTED**
- **목적**: 성능이 최우선인 상황에서 사용.
- **특징**: 
  - Dirty Read(커밋되지 않은 데이터 읽기)가 가능.
  - 가장 낮은 수준의 고립성을 제공하므로 트랜잭션 간 데이터 간섭이 많음.
- **사용 이유**:
  - 데이터 무결성보다 **속도와 성능이 중요한 경우**에 사용.
  - 예: **로그 데이터, 실시간 대시보드, 임시 통계**와 같은 데이터.  
    - 이 데이터들은 정확성이 중요하지 않고, 대략적인 데이터만 필요할 때가 많습니다.
    - 예: "현재 몇 명의 사용자가 사이트를 방문 중인지?"를 실시간으로 보여주는 대시보드에서 완벽한 데이터 정합성을 보장할 필요는 없습니다.
  - 트랜잭션 비용을 최소화하며 읽기 성능을 극대화할 수 있음.

#### **READ-COMMITTED**
- **목적**: Dirty Read 문제를 방지하면서 성능을 유지.
- **특징**:
  - 커밋된 데이터만 읽기 가능.
  - Non-Repeatable Read 문제는 여전히 존재.
- **사용 이유**:
  - **사용자 조회, 상품 리스트 조회**와 같은 작업에서 사용.
    - 사용자가 "정확한 커밋된 데이터"를 보기만 하면 되는 경우.
    - 예: 쇼핑몰 상품 리스트는 가격이 변경되거나 품절 상태가 업데이트될 수 있지만, 사용자는 "커밋된 최신 데이터"만 보면 충분.
  - 완벽한 정합성을 요구하지 않으면서도, Dirty Read 같은 명백한 데이터 오류를 방지.

---

### **2. READ-UNCOMMITTED와 READ-COMMITTED는 정말 무쓸모인가?**

#### **1) 단순 SELECT와의 차이**
- 일반적으로 `SELECT` 문만 사용한다면 트랜잭션 고립 수준이 없어도 됩니다.
- 하지만 **트랜잭션 내에서 SELECT**가 사용되는 경우, **다른 트랜잭션의 간섭을 허용할지 말지**를 결정해야 합니다.
  - 예: 다중 사용자가 동시에 데이터를 조회하고 업데이트할 때, 고립 수준이 없으면 Dirty Read나 데이터 간섭이 발생할 수 있습니다.

#### **2) 예시로 살펴보기**
##### **READ-UNCOMMITTED**
- 상황: 대시보드에서 실시간 데이터를 보여줌.
- 문제:
  - 트랜잭션 A에서 아직 커밋되지 않은 데이터를 대시보드가 읽어도 문제가 없음.
  - 예: "사용자가 100명 접속 중"이라는 데이터가 롤백되어도, 대시보드 사용자 입장에선 큰 영향이 없음.
- 이유: 성능이 더 중요한 경우 사용.

##### **READ-COMMITTED**
- 상황: 쇼핑몰 상품 조회.
- 문제:
  - 트랜잭션 A에서 상품 가격을 10만 원에서 8만 원으로 변경하고 커밋.
  - 트랜잭션 B가 상품 리스트를 조회할 때, 변경되기 전의 가격(10만 원)을 보면 안 됨.
  - READ-COMMITTED는 커밋된 데이터만 보여주므로 이런 문제를 방지.

---

### **3. 그렇다면 언제 무쓸모처럼 보일 수 있을까?**
- 현실적으로 READ-UNCOMMITTED는 잘 사용되지 않습니다.
  - Dirty Read 문제는 너무 치명적일 수 있기 때문에, 대부분의 경우 READ-COMMITTED 이상의 고립 수준을 사용합니다.
- READ-COMMITTED는 실제로 많이 사용되며, 기본 고립 수준으로 설정되는 경우가 많습니다.
  - 예: 대부분의 데이터베이스(MySQL, PostgreSQL)는 기본적으로 READ-COMMITTED를 사용.

---

### **4. 결론**
- **READ-UNCOMMITTED와 READ-COMMITTED는 무쓸모가 아닙니다.**  
  - 성능과 데이터 무결성 간의 균형이 필요한 경우 유용합니다.
  - 특히 READ-COMMITTED는 일반적인 애플리케이션에서 가장 많이 사용하는 고립 수준입니다.
- 하지만 사용 사례에 따라 중요성이 크게 달라지며, **READ-UNCOMMITTED는 매우 드물게 사용**됩니다.
- 현실적으로는 **READ-COMMITTED 이상(REPEATABLE-READ, SERIALIZABLE)**이 대부분의 애플리케이션에 적합합니다.
